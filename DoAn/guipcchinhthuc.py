...
class TTT_GUI_PC(tk.Tk):
    def __init__(self):
        ...
        self.reset_button = tk.Button(self, text='Reset', font=('Helvetica', 14), command=self.reset,foreground='red',background='yellow',relief='raised',padx=109,pady=10)
        # show
        ...
        for i in range(3):
            for j in range(3):
                self.board_labels[i][j].bind('<Button-1>', lambda event, row=i, col=j: self.click(row, col))
        self.turn_label = tk.Label(self, text="Your turn", font=('Helvetica', 14))
